...
define('MCM_PHPSTAN_PS_ROOT_CONFIG_FILE', __DIR__ . '/.phpstan_bootstrap_config.php');
...
{
    if (file_exists(MCM_PHPSTAN_PS_ROOT_CONFIG_FILE)) {
        require MCM_PHPSTAN_PS_ROOT_CONFIG_FILE;
...
function create_default_config_file_and_exit(): void
{
    if (!getenv('_PS_ROOT_DIR_')) {
        ...
        if (!file_exists(MCM_PHPSTAN_PS_ROOT_CONFIG_FILE)) {
            file_put_contents(MCM_PHPSTAN_PS_ROOT_CONFIG_FILE,
                '<?php' . PHP_EOL .
                 '// replace ' . DEFAULT_PATH_TO_PRESTASHOP . ' with a local path.' . PHP_EOL .
                'putenv(\'_PS_ROOT_DIR_=' . DEFAULT_PATH_TO_PRESTASHOP . '\');');

            throw new RuntimeException(PHP_EOL . PHP_EOL . '    Missing phpstan configuration file.' . PHP_EOL . '    Default file created. Edit file "' . MCM_PHPSTAN_PS_ROOT_CONFIG_FILE . '"' . PHP_EOL . PHP_EOL);
        }
...
function abort_on_incorrect_configuration(): void
{
    ...
    if (false === getenv('_PS_ROOT_DIR_')) {
        throw new RuntimeException(PHP_EOL . PHP_EOL . '     PS_ROOT_DIR_ not set on "' . MCM_PHPSTAN_PS_ROOT_CONFIG_FILE . PHP_EOL . '     Correct this file or delete it. ');
    }

    // default path was left
    if (DEFAULT_PATH_TO_PRESTASHOP === getenv('_PS_ROOT_DIR_')) {
        throw new RuntimeException(PHP_EOL . PHP_EOL . '     You must configure the path to Prestashop in "' . MCM_PHPSTAN_PS_ROOT_CONFIG_FILE . PHP_EOL . '     You left the default value.' . PHP_EOL);
    }

    // abort if _PS_ROOT_DIR_ is not a valid path
    if (!file_exists(getenv('_PS_ROOT_DIR_'))) {
        throw new RuntimeException(PHP_EOL . PHP_EOL . '     Invalid path to prestashop. Provided : ' . getenv('_PS_ROOT_DIR_') . PHP_EOL . ' Edit "' . MCM_PHPSTAN_PS_ROOT_CONFIG_FILE . '" and modify _PS_ROOT_DIR_ ');
    }
}
